...
import com.lxcay.laoke.activity.ShowBigImage;
...
import com.lxcay.laoke.utils.TimeUtil;
...
public class ChatMessageAdapter extends BaseAdapter {
	...
	private void handleVoiceMessage(final GotyeMessage message,
		...
		if (isPlaying) {
			...
			if (getDirect(message) == MESSAGE_DIRECT_RECEIVE) {
				holder.iv.setImageResource(R.animator.voice_from_icon);
			} else {
				holder.iv.setImageResource(R.animator.voice_to_icon);
			}
	...
	private View createViewByMessage(GotyeMessage message) {
		switch (message.getType()) {
			...
			if (user != null && user.getIcon() != null) {
				...
				if (bmp != null) {
					...
					if (bmp != null) {
						...
						iconView.setImageResource(R.mipmap.head_icon_user);
					}
				}
			} else {
				iconView.setImageResource(R.mipmap.head_icon_user);
