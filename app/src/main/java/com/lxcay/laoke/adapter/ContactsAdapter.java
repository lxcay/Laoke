...
public class ContactsAdapter extends BaseAdapter {
    ...
    public View getView(int position, View arg1, ViewGroup arg2) {
        ...
        if (arg1 == null) {
            arg1 = LayoutInflater.from(mContext).inflate(R.layout.layout_contacts_item, null);
            viewHolder = new ViewHolder();
            viewHolder.icon = (ImageView) arg1.findViewById(R.id.icon);
            viewHolder.firstChar = (TextView) arg1.findViewById(R.id.first_char);
        ...
        if (user.gotyeUser.getId() == -2) {
            ...
            String name = user.gotyeUser.getNickname();
    ...
    private void setIcon(ViewHolder viewHolder, GotyeUserProxy user) {
        if (user.gotyeUser.getIcon() != null) {
            ...
            if (user.gotyeUser.getId() == -2) {
                viewHolder.icon.setImageResource(R.mipmap.contact_room);
            } else if (user.gotyeUser.getId() == -1) {
                viewHolder.icon.setImageResource(R.mipmap.contact_group);
            } else {
                ImageCache.getInstance().setIcom(viewHolder.icon, user.gotyeUser);
            }
        } else {
            viewHolder.icon.setImageResource(R.mipmap.head_icon_user);
        }
    }
